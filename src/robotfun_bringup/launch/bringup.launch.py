...
El "conflicto/bucle" en RViz ocurre cuando dos nodos publican /joint_states a la
vez (p. ej. sliders + joint_state_publisher, o sliders + ESP32). Aquí el
argumento ``mode`` garantiza una única fuente:

  mode:=sim       (por defecto) IK/trayectoria → /joint_command → joint_state_relay
                  → /joint_states → RViz. El robot se mueve siguiendo la IK SIN
                  hardware. (Fuente única: el relay.)
...
Otros argumentos:
  model:=primitives|meshes       qué modelo visualizar.
  controller:=trajectory|ik       controlador (modos sim/hardware).
