...
la posición de /fk_pose debe ser idéntica a la del frame del efector final en TF
...
from robotfun_kinematics.core import ARM_JOINT_NAMES, ROBOT
from robotfun_kinematics.core.ik_solver import rot_to_quat
...
class FKCheckNode(Node):
    ...
    def cb(self, msg: JointState):
        ...
        self.get_logger().info(
            f"q(deg)={np.round(np.degrees(q), 1)} → EE xyz={np.round(p, 4)}",
